...
(IP) network, in particular for the *Transmission Control Protocol
(TCP)*, which is a protocol for _one-to-one_ connections. +
In this context, sockets are assumed to be associated with a specific
socket address, namely the *IP address* and a *port number* for the
local node, and there is a corresponding socket address at the foreign
node (other node), which itself has an associated socket, used by the
foreign process. Associating a socket with a socket address is called
_binding_.
