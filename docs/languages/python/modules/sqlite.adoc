...
operations because doing so is insecure: +
it makes your program vulnerable to an
https://en.wikipedia.org/wiki/SQL_injection[SQL injection attack]

Put `+?+` as a placeholder wherever you want to use a value, and then
provide a _tuple of values_ as the second argument to the cursor’s
`+execute()+` method.
