...
from arguments added to parser)
...
== https://docs.python.org/3/library/argparse.html#the-add-argument-method[Adding Arguments]
...
useful when multiple arguments need to store constants to the same list.
For example:
...
For example, this is useful for increasing verbosity levels: *Note*: the
default will be None unless explicitly set to 0.
...
*Note*: If the nargs keyword argument is not provided, the number of
arguments consumed is determined by the action.
...
*Note*: `+nargs=1+` produces a list of one item. This is different from
...
For optional arguments, there is an additional case: the option string
is present but not followed by a command-line argument. In this case the
value from const will be produced.
