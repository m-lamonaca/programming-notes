...
[width="100%",cols="13%,15%,72%",options="header",]
|==
|Attribute name |Format |Description
|asctime |`+%(asctime)s+` |Human-readable time when the LogRecord was
created. Modified by `+basicConfig(datefmt="")+`
...
|message |`+%(message)s+` |The logged message, computed as
`+msg % args+`.
...
difference from UTC/GMT of the form +HHMM or -HHMM [-23:59, +23:59].
