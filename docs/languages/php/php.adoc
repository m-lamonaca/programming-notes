...
https://www.php.net/docs.php[PHP Docs]
...
A string is a sequence of ASCII characters. In PHP a string is an array
of characters.
...
* `+__FUNCTION__+`: the function name, or \{closure} for anonymous
functions.
...
Associative arrays have a value as an index. Alternative names are _hash
tables_ or _dictionaries_.
...
[width="100%",cols="12%,15%,73%",options="header",]
|===
...
|`+xor+` |`+$a xor $b+` |TRUE if either `+$a+` or `+$b+` is TRUE, but
_not both_.
...
`+match+` can return values, doesn’t require break statements, can
combine conditions, uses strict type comparisons and doesn’t do any type
coercion.
...
`+continue+` skips one cycle of the iteration. `+exit()+` terminates the
execution of any PHP code.

=== Functions

https://make.wordpress.org/core/handbook/best-practices/inline-documentation-standards/php/[Function
Docstring]

Parameters with default values are optional in the function call and
must be the last ones in the function declaration. Return type is
optional if type checking is disabled.
...
*Union types* are a collection of two or more types which indicate that
_either_ one of those _can be used_.
...
Named arguments allow to pass in values to a function, by specifying the
value name, to avoid taking their order into consideration. It’s also
possible to skip optional parameters.
...
`+private+` methods and attributes are visible only inside the class in
which are declared. `+protected+` methods and attributes are visible
only to child classes.
...
Inside static methods it’s impossible to use `+$this+`. A static
variable is unique for the class and all instances.
...
Parameters of the dependency can be modified before passing the required
...
If a class is defined `+final+` it can’t be extended. If a function is
declared `+final+` it can’t be overridden.
...
An interface is a "`contract`" that defines what methods the
implementing classes *must* have and implement.

A class can implement multiple interfaces but there must be no methods
in common between interface to avoid ambiguity.
...
`+Traits+` allows the reutilization of code inside different classes
without links of inheritance. It can be used to mitigate the problem of
_multiple inheritance_, which is absent in PHP.

In case of functions name conflict it’s possible to use `+insteadof+` to
specify which function to use. It’s also possible to use an _alias_ to
...
* `+md2+`, `+md4+`, `+md5+`
* `+sha1+`, `+sha224+`, `+sha256+`, `+sha384+`, `+sha512/224+`,
`+sha512/256+`, `+sha512+`
* `+sha3-224+`, `+sha3-256+`, `+sha3-384+`, `+sha3-512+`
* `+ripemd128+`, `+ripemd160+`, `+ripemd256+`, `+ripemd320+`
* `+whirlpool+`
* `+tiger128,3+`, `+tiger160,3+`, `+tiger192,3+`, `+tiger128,4+`,
`+tiger160,4+`, `+tiger192,4+`
* `+snefru+`, `+snefru256+`
* `+gost+`, `+gost-crypto+`
* `+adler32+`
* `+crc32+`, `+crc32b+`, `+crc32c+`
...
`+password_hash()+` is compatible with `+crypt()+`. Therefore, password
hashes created by `+crypt()+` can be used with `+password_hash()+`.

Algorithms currently supported:

* *PASSWORD_DEFAULT* - Use the _bcrypt_ algorithm (default as of PHP
5.5.0). Note that this constant is designed to change over time as new
and stronger algorithms are added to PHP.
...
"`latexmath:[2y]`" identifier. The result will always be a 60 character
...
Argon2 support.
* *PASSWORD_ARGON2ID* - Use the *Argon2id* hashing algorithm to create
the hash. This algorithm is only available if PHP has been compiled with
Argon2 support.
...
automatically generated. If omitted, a random salt will be generated by
password_hash() for each password hashed. This is the intended mode of
operation. *Warning*: The salt option has been deprecated as of PHP
7.0.0. It is now preferred to simply use the salt that is generated by
default.
* *cost* (integer) - which denotes the algorithmic cost that should be
used. Examples of these values can be found on the crypt() page. If
omitted, a default value of 10 will be used. This is a good baseline
cost, but you may want to consider increasing it depending on your
hardware.

*Supported options for PASSWORD_ARGON2I and PASSWORD_ARGON2ID*:

* *memory_cost* (integer) - Maximum memory (in kibibytes) that may be
used to compute the Argon2 hash. Defaults to
PASSWORD_ARGON2_DEFAULT_MEMORY_COST.
* *time_cost* (integer) - Maximum amount of time it may take to compute
the Argon2 hash. Defaults to PASSWORD_ARGON2_DEFAULT_TIME_COST.
* *threads* (integer) - Number of threads to use for computing the
...
* *Warning*: generated at runtime, does not stop the execution
(non-blocking).
...
https://www.php.net/manual/en/errorfunc.constants.php[PHP Error
Constants]

Its possible to configure PHP to signal only some type of errors. Errors
below a certain levels are ignored.
...
==== https://www.php.net/manual/en/function.error-log.php[Writing in the Log File]
