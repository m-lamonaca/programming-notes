...
* `+Microsoft.EntityFrameworkCore.Tools+` to use migrations in Visual
Studio
* `+Microsoft.EntityFrameworkCore.Tools.DotNet+` to use migrations in
`+dotnet+` cli (`+dotnet-ef+`)
* `+Microsoft.EntityFrameworkCore.Design+` _or_
`+Microsoft.EntityFrameworkCore.<db_provider>.Design+` needed for tools
to work (bundled w tools)
...
https://stackoverflow.com/a/5385288[Referenced Object Not Loading Fix]
