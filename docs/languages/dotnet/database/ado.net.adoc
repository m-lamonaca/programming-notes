== https://docs.microsoft.com/en-us/dotnet/framework/data/adonet/[ADO.NET]

`+ADO.NET+` is a set of classes that expose data access services for
.NET. The `+ADO.NET+` classes are found in `+System.Data.dll+`, and are
integrated with the XML classes found in `+System.Xml.dll+`.

https://system.data.sqlite.org/index.html/doc/trunk/www/index.wiki[ADO.NET
provider for SQLite]

=== https://www.connectionstrings.com[Connection Strings]

==== https://www.connectionstrings.com/sql-server-2019/[SQL Server 2019]
...
*Note*: _Multiple Active Result Sets_ (MARS) is a feature that works
with SQL Server to allow the execution of multiple batches on a single
connection.

==== https://www.connectionstrings.com/sqlite/[SQLite]
