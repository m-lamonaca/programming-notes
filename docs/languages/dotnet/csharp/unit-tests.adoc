...
https://stackoverflow.com/a/5666591/8319610[UnitTest Overloaded Methods]
https://osherove.com/blog/2005/4/3/naming-standards-for-unit-tests.html[Naming
...
class will be run for every single test. +
This makes the constructor a convenient place to put reusable context
setup code.

For context cleanup, add the `+IDisposable+` interface to the test
class, and put the cleanup code in the `+Dispose()+` method.
