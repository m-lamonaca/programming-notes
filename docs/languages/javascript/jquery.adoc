...
of the Internet content today, including web objects (text, graphics and
...
== https://api.jquery.com/category/selectors/[Finding DOM elements]
...
== https://api.jquery.com/category/events/[Events]
...
In the HTML, add a `+<script>+` ag that hotlinks to the CDN or source
file:
...
*Note*: always link to the
https://developers.google.com/speed/docs/insights/MinifyResources[minified]
js files.
...
DOM manipulation and event binding doesn’t work if the `+<script>+` is
in the `+<head>+`
