...
The *Document Object Model* is a _map_ of the HTML document. Elements in
an HTML document can be accessed, changed, deleted, or added using the
DOM. The document object is _globally available_ in the browser. It
allows to access and manipulate the DOM of the current web page.
...
`+querySelectorAll()+` return a collection of elements.
...
It’s possible access and change the attributes of a DOM node using the
_dot notation_.
...
It’s possible to access and change the styles of a DOM nodes via the
*style* property. CSS property names with a `+-+` must be *camelCased*
and number properties must have a unit.
...
Each DOM node has an `+innerHTML+` attribute. It contains the HTML of
all its children.
...
To change the actual text of a node, `+textContent+` may be a better
choice:
