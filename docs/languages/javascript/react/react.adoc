...
Both types can be stateful and have side effects or be purely
presentational.
...
_NOTE_: a component name _must_ start with an uppercase letter.

Every components has two inputs: _props_ and _state_. The props input is
explicit while the state is implicit. +
State is used to determine the changes and when to re-render. +
Within the component state can be changed while the props object
represent fixed input values.

JSX syntax can represent HTML but gets converted to pure JavaScript
...
NOTE: The `+key+` attribute of the component is needed to identify a
particular item. It’s most useful if the list has to be sorted.
...
Hook used to trigger an action on each render of the component or when
one of the watched items changes.
