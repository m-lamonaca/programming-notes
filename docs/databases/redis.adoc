...
called a value, inside a key. This data can later be retrieved only if
we know the exact key used to store it.

Often Redis it is called a _data structure_ server because it has outer
key-value shell, but each value can contain a complex data structure,
such as a string, a list, a hashes, or ordered data structures called
sorted sets as well as probabilistic data structures like _hyperloglog_.
...
==== https://redis.io/commands#generic[Key-Value Pairs]
...
==== https://redis.io/commands#list[Lists]
...
==== https://redis.io/commands#set[Sets]

A set is similar to a list, except it does not have a specific order and
...
==== https://redis.io/commands#sorted_set[Sorted Sets]
...
==== https://redis.io/commands#hash[Hashes]
