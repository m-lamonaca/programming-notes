...
Connect to the final destination jumping through the specifies other
destinations.
...
.ssh-tunnels
image::https://iximiuz.com/ssh-tunnels/ssh-tunnels.png[ssh-tunnels,title="SSH Tunnels Cheat Sheet By Ivan Velichko"]

==== Local Port Forwarding

Start listening on `+local_address:local_port+` and forward any traffic
...
Start listening on `+remote_address:remote_port+` and forward any
traffic to `+local_address:local_port+`.
